...
getCoeff <- function(dfForCoeff, forFakeTable, firstBreakpoint) {
  ...
  if (method == "lm") {
    ...
    return(coef(lm.beta(lm(Output ~ ., data=dfForCoeff )))["MnARevenue"])
...
covariates <- df %>% select(FirmName, Year, Lawyers, Lawyers2, Leverage, 
                           MnARevenue, EquityRevenue, IPORevenue, 
                           MnANumOfDeals, EquityIssues, IPOIssues)
# Note: MnANumOfDeals = MnAIssues
# lawyers, lawyers^2, leverage, intercept, mnarevenue, equityrevenue, iporevenue, ipoissues, equityissues, mnaissues
